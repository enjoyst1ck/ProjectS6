...
export default function NavAddPage({ currStep, setCurrStep }) {
   ...
   const jumpBetwennCard = (curr) => {
      console.log('44456654');
   };
   return (
      <div className='w-full flex justify-between items-center'>
         ...
         <div className='flex '>
            {helper.map((item, index) => (
               <div key={index} onClick={() => jumpBetwennCard(index)}  className={index === currStep - 1 ? 'size-6 rounded-full border-4 bg-[#be3144ec] border-[#be3144ec] mx-1' : 'size-6 rounded-full border-4 border-[#be3144ec] mx-1'}></div>
            ))}
         </div>
